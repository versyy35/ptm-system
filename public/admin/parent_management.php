...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'create') {
            $stmt = $pdo->prepare("INSERT INTO parents (name, email, phone, isams_parent_id) VALUES (?, ?, ?, ?)");
            $stmt->execute([
                $_POST['name'],
                $_POST['email'],
                $_POST['phone'] ?? null,
            ...
            $stmt = $pdo->prepare("UPDATE parents SET name=?, email=?, phone=?, isams_parent_id=? WHERE id=?");
            $stmt->execute([
                $_POST['name'],
                $_POST['email'],
                $_POST['phone'] ?? null,
            ...
            $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM students WHERE parent_id = ?");
            $checkStmt->execute([$_POST['id']]);
            if ($checkStmt->fetchColumn() > 0) {
                $errorMessage = "Cannot delete parent with assigned students. Please reassign students first.";
            } else {
...
<body>
    <div class="container-fluid">
        <div class="management-header">
            <div class="row align-items-center">
                <div class="col">
                    <h2><i class="fas fa-users"></i> Parents Management</h2>
                    <p class="text-muted mb-0">Manage parent records</p>
                </div>
                <div class="col-auto">
        ...
        <div class="nav-links">
            ...
            <a href="?page=parent-management" class="active"><i class="fas fa-users"></i> Parents</a>
        ...
        <div class="content-card">
            ...
            <div class="action-bar">
                <input type="text" id="searchBox" class="search-box" placeholder="🔍 Search parents...">
            ...
            <?php if (empty($parents)): ?>
                <div class="empty-state">
                    ...
                    <p>Click "Add Parent" to create your first parent record.</p>
                </div>
            <?php else: ?>
                <div style="overflow-x: auto;">
                    <table class="data-table">
                        ...
                        <tbody id="dataTableBody">
                            ...
                            <tr class="data-row">
                                <td><strong><?php echo htmlspecialchars($parent['name']); ?></strong></td>
                                <td><?php echo htmlspecialchars($parent['email']); ?></td>
                                <td><?php echo htmlspecialchars($parent['phone'] ?: 'N/A'); ?></td>
                                <td><span class="badge badge-info"><?php echo $parent['student_count']; ?> student(s)</span></td>
                                <td><?php echo htmlspecialchars($parent['isams_parent_id'] ?: 'N/A'); ?></td>
                                <td>
                                    <button class="btn-edit" onclick='openEditModal(<?php echo json_encode($parent); ?>)'>
                                        <i class="fas fa-edit"></i> Edit
                                    </button>
                                    <button class="btn-delete" onclick="confirmDelete(<?php echo $parent['id']; ?>, '<?php echo htmlspecialchars($parent['name']); ?>')">
    ...
    <div id="parentModal" class="modal">
        <div class="modal-content">
            <h3 id="modalTitle"><i class="fas fa-users"></i> Add Parent</h3>
            <form method="POST" id="parentForm">
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" id="parentEmail" required>
                </div>

                <div class="form-row">
                    <div class="form-group">
                        <label for="parentPhone">Phone:</label>
                        <input type="tel" name="phone" id="parentPhone" placeholder="Optional">
                    </div>
                    <div class="form-group">
                        <label for="parentIsamsId">ISAMS Parent ID:</label>
                        <input type="number" name="isams_parent_id" id="parentIsamsId" placeholder="Optional">
                    </div>
                </div>
    ...
    <script>
        ...
        function openCreateModal() {
            ...
            document.getElementById('modalTitle').innerHTML = '<i class="fas fa-users"></i> Add Parent';
            document.getElementById('parentModal').classList.add('active');
        }

        function openEditModal(data) {
            document.getElementById('action').value = 'update';
            document.getElementById('modalTitle').innerHTML = '<i class="fas fa-users"></i> Edit Parent';
            ...
            document.getElementById('parentIsamsId').value = data.isams_parent_id || '';
            document.getElementById('parentModal').classList.add('active');
        }
        ...
        function closeModal(modalId = 'parentModal') {
